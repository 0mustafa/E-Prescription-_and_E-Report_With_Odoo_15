...
#
#
# -*- Kırmızı Reçete ile Kaydedilebilen İlaç Listesi Servisi -*-
#
#
import datetime
...
class KirmiziReceteIlacListesiSorgula(models.Model):
    _name = "service.kirmizi_ilac_listesi"
    _description = "kirmizi Reçete İle Kaydedilebilen İlaç Listesi Servisi"

    tesis_kod = fields.Char(string="Tesis Kodu")
    doctor_id = fields.Many2one('hospital.doctor', string="Doktor")
    islem_tarihi = fields.Date(string="İşlem Tarihi", default=fields.Date.context_today, date_format="dd.MM.yyyy",
                               readonly=True)
    ilac_listesi = fields.Many2many('hospital.ilac', string="İlaç Listesi")

    def kirmizi_ilac_listesi_sorgula(self):
        ...
        with client.settings(strict=False):
            ilac_listesi = client.service.kirmiziReceteIlacListesiSorgula(**vals)
        result = ilac_listesi.ilacListesi

        if ilac_listesi.sonucKodu == '0000':
            my_ilac_barcode = self.env['hospital.ilac'].search([]).mapped('barcode')
            not_in_my_ilac = filter(lambda r: str(r.barkod) not in my_ilac_barcode, result)
            for ilac in not_in_my_ilac:
                ...
                my_ilac = self.env['hospital.ilac'].search([('barcode', '=', str(ilac.barkod))])

                self.ilac_listesi = [(4, my_ilac.id, 0)]

            return {
                'name': 'Yeni Eklenen İlaçlar',
