...
class QueryEreport(models.Model):
    _name = "hospital.service.query.ereport"
    _description = "Hospital Query E-Report Service"

    rapor_takip_no = fields.Char(string="E-Rapor Takip No")
    ...
    def query_ereport(self):
        ...
        if erapor.sonucKodu == '0000':
            ...
            for doktor in result.eraporDoktorListesi:
                # doktor = self.env['hospital.doctor'].search([('')])
            ...
            for etkin_madde in result.eraporEtkinMaddeListesi:
                etkin_madde_listesi.append({
                    'etkin_maddedvo_id': self.env['hospital.etkin_maddedvo'].search([('etkin_madde_kodu', '=', etkin_madde['etkinMaddeDVO'].kodu)]).id,
            ...
            for teshis in result.eraporTeshisListesi:
                ...
                teshis_listesi.append({
                    'rapor_teshis_kodu': self.env['hospital.ereport.teshis'].search([('rapor_teshis_kodu', '=', str(teshis.raporTeshisKodu))], limit=1).id,
            ...
            return {
                ...
                'res_model': 'hospital.ereport.wizard',
                'target': 'new',
                'view_type': 'form',
                'view_mode': 'form',
                'context': {
                    ...
                    'default_rapor_takip_no': result.raporTakipNo,
                    ...
                    'default_patient_id': self.env['hospital.epatient'].search([('tc_no', '=', result.hastaTcKimlikNo)], limit=1).id,
